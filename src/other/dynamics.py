...
def f_casadi(s, u):
    ...
    s_dot = [None] * 6
    s_dot[0] = x_dot
    s_dot[1] = 1/m * (f_e*ca.sin(phi + theta) + f_s*ca.cos(theta))
    s_dot[2] = z_dot
    s_dot[3] = 1/m * (f_e*ca.cos(phi + theta) - f_s*ca.sin(theta) - m*g)
    s_dot[4] = theta_dot
    s_dot[5] = 1/inert_tensr * (l2*f_s - f_e*ca.sin(phi)*(l1 + ca.cos(phi)*ln))
    # s_dot[6] = -alpha * f_e - beta * ca.abs(f_s)

    s_dot = ca.vertcat(s_dot[0], s_dot[1], s_dot[2], s_dot[3], s_dot[4], s_dot[5])
    return s_dot
